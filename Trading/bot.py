...
def request_stock_price_hist(symbol, token, sample = False):
    ...
    """
    if sample == False:
        q_string = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol={APPL}&outputsize=full&apikey={api_key}'
    else:
        q_string = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol={APPL}&apikey={api_key}'
...
def request_symbols(token):
    ...
    df : Pandas DataFrame
        A Pandas DataFrame containing stock price information.
    """
    q_string = 'https://www.alphavantage.co/query?function=LISTING_STATUS&apikey={api_key}'   
